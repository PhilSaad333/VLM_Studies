...
def build_uid_index(dataset: Dataset) -> Dict[str, int]:
    """Return a mapping from UID to dataset index."""

    if isinstance(dataset, IterableDataset):
        raise ValueError("UID index requires a non-streaming Dataset")

    uids = dataset["uid"]
    return {uid: idx for idx, uid in enumerate(uids)}


def find_sample_by_uid(
    dataset: Dataset | IterableDataset,
    uid: str,
    limit: Optional[int] = None,
    uid_index: Optional[Dict[str, int]] = None,
) -> Dict[str, Any]:
    if uid_index is not None and not isinstance(dataset, IterableDataset):
